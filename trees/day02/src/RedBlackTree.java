...
public class RedBlackTree<T extends Comparable<T>> extends BinarySearchTree<T> {
    ...
    private TreeNode<T> balance(TreeNode<T> h) {
        // TODO
        if (isRed(h.rightChild)==true) {
            h = rotateLeft(h);
        }
        if (isRed(h.leftChild) == true && isRed(h.leftChild.leftChild) == true) {
            h = rotateRight(h);
        }
        if (isRed(h.leftChild) == true && isRed(h.rightChild)== true) {
            h = flipColors(h);
        }
        return h;
    }
    ...
    TreeNode<T> insert(TreeNode<T> h, T key) {
        ...
        // TODO: use balance to correct for the three rotation case
        return balance(h);
    }
